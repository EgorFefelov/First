...
function makeCanvasTruck() {
  ...
  const dash = box(1.85, .42, .65, 0x16181b, .85, .15);
  dash.position.set(0, 1.58, -2.95);
  dash.rotation.x = -0.2;
  cockpit.add(dash);

  steeringWheel = new THREE.Group();
  const wRing = new THREE.Mesh(new THREE.TorusGeometry(.22, .03, 10, 24), mat(0x111214, .9, .1));
  steeringWheel.add(wRing);
  const wSpoke = box(.21, .028, .02, 0x111214);
  steeringWheel.add(wSpoke);
  const wSpokeV = box(.028, .19, .02, 0x111214); wSpokeV.position.y = .09;
  steeringWheel.add(wSpokeV);
  steeringWheel.position.set(-.46, 1.78, -2.62);
  steeringWheel.rotation.x = -0.52;
  cockpit.add(steeringWheel);

  const speedoGlow = new THREE.Mesh(new THREE.CircleGeometry(.1, 14), new THREE.MeshBasicMaterial({ color: 0x00f0ff }));
  speedoGlow.position.set(-.46, 1.72, -2.75);
  speedoGlow.rotation.x = -0.2;
  cockpit.add(speedoGlow);
...
function animate(now=0) {
  ...
  truck.position.x=THREE.MathUtils.lerp(truck.position.x,playerX,.18); truck.rotation.z=THREE.MathUtils.lerp(truck.rotation.z,-steer*.08,.14); if (steeringWheel) {
...
window.Transport3D={start(route){this.stop();bullets.length=0;bonuses.length=0;activeRoute=route;cameraMode=0;resetCanvas();buildWorld(route);running=true;speed=15;playerX=0;lives=3;shotClock=0;bonusClock=0;damageCooldown=0;speedBoostCount=0;speedBoostUntil=0;fuel=1;crateCounts={ammo:0,weapon:0,grenade:0};document.querySelectorAll("[data-crate-count]").forEach(el=>el.textContent="0");spawnBonus("fuel");bonuses[0].position.z=-42;updateLives();window.GameAudio?.playMode("transport");animate.last=performance.now();frame=requestAnimationFrame(animate);},stop(){running=false;window.GameAudio?.stop();if(frame)cancelAnimationFrame(frame);if(renderer)renderer.dispose();},toggleCamera(){cameraMode=cameraMode===0?1:0;const camLabel=document.getElementById("transport-camera");if(camLabel)camLabel.textContent=cameraMode===0?"Камера [V]: 3-е лицо (Сзади)":"Камера [V]: 1-е лицо (Кабина)";const camBtn=document.getElementById("transport-cam-mode");if(camBtn)camBtn.textContent=cameraMode===0?"Камера [V]: 3-е лицо":"Камера [V]: Кокпит";},isRunning(){return running;}};
})();
